...
// Version 1: precomputed sin/cos tables
template<int BLOCK_N>
__global__ void attention_fwd_kernel_sinusoidal_fused(
    const float* __restrict__ Q,
    const float* __restrict__ K,
    const float* __restrict__ V,
    const float* __restrict__ sin_table,  //precomputed table of sin embeddings
    const float* __restrict__ cos_table, //precomputed table of cos embeddings
    float* __restrict__ out,
    int bs, int nh, int seq_q, int seq_k, int hdim,
    float scale_factor
) {
    ...
    for (int k_start = 0; k_start < seq_k; k_start += BLOCK_N) {
        ...
        for (int k_local = tid; k_local < num_k; k_local += blockDim.x) {
            ...
            for (int d = 0; d < hdim; d++) {
                ...
                // Add positional encoding from table
                if (d % 2 == 0) {
...
// Version 2: float4 vectorization with precomputed table
template<int BLOCK_N>
...
// Version 3: compute sin/cos in-kernel rather than using table
template<int BLOCK_N>
...
// Version 4: cached frequencies to reduce exp calls
template<int BLOCK_N>
...
// attention kernel (for separate approach)
template<int BLOCK_N>
...
void test_sinusoidal_attention(int bs, int nh, int seqlen, int hdim) {
    ...
    for (int i = 0; i < num_iters; i++) {
        ...
        attention_kernel<BLOCK_N><<<grid_attn, nthreads, shmem_sz>>>(
