...
// Kernel to add sinusoidal embeddings to K (pre-computation)
__global__ void add_sinusoidal_to_k(
    ...
    // Float4 vectorization for better memory throughput
    if (hdim % 4 == 0) {
        for (int d = threadIdx.x * 4; d < hdim; d += blockDim.x * 4) {
            int offset = token_idx * hdim + d;
            int sin_offset = pos * hdim + d;

            float4 k_vec = *reinterpret_cast<const float4*>(&K[offset]);
            ...
            *reinterpret_cast<float4*>(&K_emb[offset]) = k_vec;
        }
    } else {
...
// Hybrid sinusoidal attention kernel
// K already has embeddings added, fuse Q embeddings into attention
template<int BLOCK_N>
__global__ void attention_fwd_kernel_sinusoidal_hybrid(
    ...
    float* s_q_emb = s_reduce + 32;  // Cache Q with embeddings

    const int q_offset = qkv_base + q_idx * hdim;
    const int sin_offset = q_idx * hdim;

    // Pre-compute Q embeddings and cache in shared memory
    if (hdim % 4 == 0) {
        for (int d = tid * 4; d < hdim; d += blockDim.x * 4) {
            ...
            // Apply sin to even indices, cos to odd indices
            q_vec.x += sin_vec.x;  // d+0 is even
            q_vec.y += cos_vec.y;  // d+1 is odd
            q_vec.z += sin_vec.z;  // d+2 is even
            q_vec.w += cos_vec.w;  // d+3 is odd

            *reinterpret_cast<float4*>(&s_q_emb[d]) = q_vec;
        }
    } else {
        for (int d = tid; d < hdim; d += blockDim.x) {
            float val = Q[q_offset + d];
            if (d % 2 == 0) {
                val += sin_table[sin_offset + d];
            } else {
                val += cos_table[sin_offset + d];
            }
            s_q_emb[d] = val;
        }
    }
    ...
    for (int k_start = 0; k_start < seq_k; k_start += BLOCK_N) {
        ...
        // Compute Q@K^T scores (K already has embeddings)
        for (int k_local = tid; k_local < num_k; k_local += blockDim.x) {
            ...
            // Vectorized dot product with float4
            if (hdim % 4 == 0) {
                #pragma unroll 4
                for (int d = 0; d < hdim; d += 4) {
                    float4 q_vec = *reinterpret_cast<const float4*>(&s_q_emb[d]);
                ...
                for (int d = 0; d < hdim; d++) {
                    score += s_q_emb[d] * K_emb[k_offset + d];
                }
            }
...
// Forward function for hybrid approach
void attention_forward_sinusoidal_hybrid(
    const float* Q,
    const float* K,
    const float* V,
    ...
    add_sinusoidal_to_k<<<total_k, 256>>>(K, sin_table, cos_table, K_emb, total_k, head_d, len_k);

    // Step 2: Run hybrid attention (fuses Q embeddings)
    constexpr int BLOCK_N = 64;
    dim3 grid(len_q, batch_sz * num_heads);
    const int nthreads = 256;
    const int shmem_sz = (BLOCK_N + 32 + head_d) * sizeof(float);

    attention_fwd_kernel_sinusoidal_hybrid<BLOCK_N><<<grid, nthreads, shmem_sz>>>(
...
// Separate approach (baseline) - for comparison
...
// Standard attention kernel (for separate baseline)
template<int BLOCK_N>
...
void benchmark_hybrid_vs_separate(int bs, int nh, int seqlen, int hdim) {
    printf("\n========================================\n");
    printf("BENCHMARK: Hybrid vs Separate Sinusoidal\n");
    ...
    // Warmup
    attention_forward_sinusoidal_hybrid(d_Q, d_K, d_V, d_sin, d_cos, d_K_emb, d_out, bs, nh, seqlen, seqlen, hdim);
    ...
    printf("\n--- Performance Results ---\n");
    printf("  Hybrid (pre-K, fuse-Q):   %.3f ms", hybrid_ms);
...
int main() {
    printf("=================================================\n");
    printf("Sinusoidal Hybrid Attention Kernel Tests\n");
    printf("Comparing Hybrid (pre-K + fuse-Q) vs Separate\n");
    printf("=================================================\n");
    ...
    printf("\n=================================================\n");
    printf("Hybrid sinusoidal tests completed!\n");
    printf("=================================================\n");
